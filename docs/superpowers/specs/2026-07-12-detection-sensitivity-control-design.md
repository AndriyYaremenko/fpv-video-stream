...
- scancfg не прийшов (агент офлайн/стара версія) → панель для цього сканера НЕ показується (реалізовано:
  `scannerThresholdCards` вимагає online+scancfg; стара версія все одно не приймає threshold-команди, а
  ретейн-scancfg присутній на mount у актуального агента).
