...
export function diffDetections(prevByKey, scannerId, payload, isBaseline) {
  ...
  for (const d of dets) {
    if (d && typeof d === 'object') current.set(detectionKey(d), d);   // skip null/garbage entries
  }
  const events = [];
  if (!isBaseline) {
...
export class DetectionJournal {
  constructor({ file = '', max = 2000, maxScanners = 64 } = {}) {
    this._file = file;
    this._max = max;
    this._maxScanners = maxScanners;
    this._events = [];                 // oldest..newest
    this._byScanner = new Map();       // scannerId -> Map(key -> detection); LRU-ordered, capped
    this._load();
  }

  ingest(scannerId, payload) {
    const isBaseline = !this._byScanner.has(scannerId);
    const prev = this._byScanner.get(scannerId) || new Map();
    const { events, current } = diffDetections(prev, scannerId, payload, isBaseline);
    this._byScanner.delete(scannerId);            // re-insert at the end → LRU order
    this._byScanner.set(scannerId, current);
    // Bound memory against spoofed scanner ids: FIFO-evict the least-recently-seen scanner.
    if (this._byScanner.size > this._maxScanners) {
      this._byScanner.delete(this._byScanner.keys().next().value);
    }
    if (events.length) {
  ...
  // Synchronous full-ring write per events-producing ingest. Events are infrequent
  // (appear/disappear, not per-cycle) and the ring is capped, so the O(n) write is fine.
  _persist() {
    if (!this._file) return;
